...
QVariant SessionDetailModel::data(const QModelIndex &index, int role) const {
    ...
    case NoteRole:
        return ex.note;
    case EffortRirRole:
        return ex.effortRir.has_value() ? QVariant(ex.effortRir.value()) : QVariant();
    case SetsRole:
...
bool SessionDetailModel::loadSession(int sessionId) {
    ...
    const bool hasRir = tableHasColumn(db, QStringLiteral("exercises"), QStringLiteral("effort_rir"));

    QSqlQuery exQuery(db);
    const QString exSql = QStringLiteral(
        "SELECT id, %1 AS machine_name_snapshot, comment, machine_id, %2 AS effort_rir "
        "FROM exercises WHERE session_id = ? %3")
                              .arg(hasSnapshot ? QStringLiteral("machine_name_snapshot") : QStringLiteral("NULL"))
                              .arg(hasRir ? QStringLiteral("effort_rir") : QStringLiteral("NULL"))
    ...
    while (exQuery.next()) {
        ...
        if (hasRir && !exQuery.value(4).isNull()) {
            ex.effortRir = exQuery.value(4).toInt();
        }

        QSqlQuery setQuery(db);
        setQuery.prepare(QStringLiteral(
            "SELECT set_number, reps, weight_lbs FROM exercise_sets WHERE exercise_id = ? ORDER BY set_number"));
